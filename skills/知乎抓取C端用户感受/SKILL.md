...
- 推送：报告通过大小和结构校验后，由 007 创建钉钉在线文档并回读校验，按目标群当日成员授予只读权限；随后只把标题、文档链接、报告日期和幂等键交给本机 n8n，由 n8n 调用群机器人 Webhook 发送“标题 + 点击查看完整市场情报”。完整报告正文不经过 n8n 或群机器人。文档、访问权限、n8n 投递和整体完成分别写入进度标记，相同日期和文件摘要不重复执行。
...
- `server/send_dingtalk.py`：创建并回读钉钉在线文档，向目标群成员授予只读权限，再调用 007 本机 n8n Webhook 投递标题和入口；支持阶段恢复与整体幂等。
